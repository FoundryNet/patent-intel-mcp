...
logger = logging.getLogger("patent.pay")
...
def payment_required_body(tool: str, intent: str, price_usdc: float,
    ...
    n_servers = getattr(config, "NETWORK_SERVER_COUNT", 21)
...
async def verify_payment(tx_signature: str, expected_memo: str, price_usdc: float) -> dict:
    ...
    if delta < need:
        return _fail("underpaid", f"Transferred {delta / 10**_USDC_DECIMALS:.6f} USDC; need at least {price_usdc:.2f} USDC.")
